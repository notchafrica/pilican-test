<x-livewire-tables::table wire:sortable="{{ $reordering ? $reorderingMethod : '' }}"
    ...
    <x-slot name="head">
        ...
        <x-livewire-tables::table.heading>
            <div class="inline-flex rounded-md shadow-sm">
                <input wire:model="selectPage" type="checkbox"
                    class="rounded border-gray-300 text-blue-600 shadow-sm transition duration-150 ease-in-out focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 dark:bg-gray-900 dark:text-white dark:border-gray-600 dark:hover:bg-gray-600 dark:focus:bg-gray-600" />
            </div>
        </x-livewire-tables::table.heading>
        @endif
        ...
        <x-livewire-tables::table.heading />
        @else
        <x-livewire-tables::table.heading :sortingEnabled="$sortingEnabled" :sortable="$column->isSortable()"
            :column="$column->column()" :direction="$column->column() ? $sorts[$column->column()] ?? null : null"
            :text="$column->text() ?? ''" :class="$column->class() ?? ''" :customAttributes="$column->attributes()" />
        @endif
        @endif
        @endforeach
    </x-slot>
    ...
    <x-slot name="customSecondaryHead">
        <x-livewire-tables::table.row wire:loading.class.delay="opacity-50 dark:bg-gray-900 dark:opacity-60"
    ...
    <x-slot name="body">
        ...
        <x-livewire-tables::table.row wire:loading.class.delay="opacity-50 dark:bg-gray-900 dark:opacity-60"
            ...
            :target="method_exists($this, 'getTableRowUrlTarget') ? $this->getTableRowUrlTarget($row) : '_self'" :class="
                    ($index % 2 === 0 ?
