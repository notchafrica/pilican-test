    ...
    <div class="flex-none h-screen w-72 pt-4 border-r px-2">
        <div class="shadow rounded-md p-3 bg-white">
            <div class="flex flex-col justify-center space-y-2">
                ...
                <div>
                    <h2 class="text-xl text-center font-semibold">{{$provider->name}}</h2>
                </div>
                <div>
                    <h3 class="text-center">@lang("Balance"): <span
                            class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-blue-600 bg-blue-50 rounded-md">
                            {{$provider->balance}}</span></h3>
                </div>
            </div>
        </div>
        <div class="px-2 w-full mt-4 space-y-2">
            <h2 class="border-b pb-2">@lang("Provider")</h2>
            <h3>@lang("Operations")</h3>
            <ul class="flex flex-col space-y-2 mt-2">
                {{-- <li><button
                        wire:click="$emit('openModal', 'provider.actions.add-credit', {{json_encode(['provider' => $provider->id])}})"
                        class="flex px-3 py-2 transition-all delay-100 items-center space-x-2 ease-in-out hover:bg-blue-100 w-full text-blue-600 rounded-md">
                        <x-icon name="credit-card" class="h-5"></x-icon> <span>@lang("Add
                            credit")</span>
                    </button></li> --}}
                <li><button
                        wire:click="$emit('openModal', 'provider.edit', {{json_encode(['provider' => $provider->id])}})"
                        class="flex px-3 py-2 transition-all delay-100 items-center space-x-2 ease-in-out hover:bg-blue-100 w-full text-blue-600 rounded-md">
    ...
    <div class="flex-grow overflow-y-auto px-4 pt-4 space-y-2">
        ...
        <div class="flex flex-col space-y-2 bg-white px-3 py-1.5 rounded-md border">
            <div class="flex items-center justify-between">
                <div
                    class="flex items-center @if($transaction->type == 'credit') text-blue-600 @else text-red-600 @endif">
